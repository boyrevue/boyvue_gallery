  ...
  return (
    <div className="login-overlay" onClick={onClose}>
      <div className="login-modal" onClick={e => e.stopPropagation()}>
        ...
        <p className="login-subtitle">Save your favorites across all devices</p>
        ...
        {(mode === 'email' || mode === 'register') && (
          <form onSubmit={handleEmailSubmit}>
            ...
            <input type="password" placeholder="Password" value={password} onChange={e => setPassword(e.target.value)} className="login-input" required minLength={6} />
            {error && <p className="login-error">{error}</p>}
            <button type="submit" className="login-btn login-btn-primary">
              {mode === 'register' ? 'Create Account' : 'Sign In'}
            </button>
            <button type="button" onClick={() => setMode('social')} className="login-btn login-btn-secondary">
