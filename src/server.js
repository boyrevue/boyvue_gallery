...
// Session for OAuth state (minimal config)
app.use(session({
...
app.use((req, res, next) => {
  ...
  req.isCreativesDomain = host === 'fans.boyvue.com' || host === 'creatives.localhost';
  req.isHubDomain = host === 'hub.boyvue.com' || host === 'hub.localhost';
  next();
});
...
// Creatives subdomain routes (also accessible via /api/creatives on main domain)
app.use('/api/creatives', creativesRoutes);
app.use('/api/creatives/admin', creativesAdminRoutes);

// Authentication routes (works across all subdomains)
app.use('/api/auth', authRoutes);

// User favorites routes (Hot or Not feature)
app.use('/api/favorites', favoritesRoutes);

// Affiliate link redirect - works across all domains
...
app.get('*', (req, res) => {
  ...
  const knownRoutes = /^\/($|v\/\d+|c\/\d+|search|categories|popular|recent|seo-dashboard)/;
...
// Admin-only server on port 9000 (bypasses Cloudflare)
// Access at: http://65.181.125.120:9000/seo-dashboard
